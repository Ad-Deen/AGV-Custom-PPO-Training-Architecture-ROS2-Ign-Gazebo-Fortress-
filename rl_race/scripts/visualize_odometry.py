...
class LidarOdomVisualizer(Node):
    ...
    def is_back_to_initial_pose(self):
        if self.initial_position is None:
            return False
        """
        If we want the bot to circulate the full lap than we finish mapping after ereturning to ionitial point. If we want to mp upto a cirtain point. 
        If we simply use the custo coordinates of theat point to exterminate the maping process
        """
        ...
        #Custom termination coordinates = (145.31 , 34.684)
        # distance = np.sqrt(
        #     (self.position.x - 145.31) ** 2 +
        #     (self.position.y - 34.684) ** 2
        # )
    ...
    def save_map(self):
        ...
        plt.scatter(self.lidar_points_x, self.lidar_points_y, s=0.01, color='black')
        ...
        save_path_png = os.path.expanduser('~/ros2_ws/src/rl_race/scripts/rl_map.png')
        ...
        # Save the LiDAR points as a .npy file
        save_path_npy = os.path.expanduser('~/ros2_ws/src/rl_race/scripts/lidar_points.npy')
        np.save(save_path_npy, np.vstack((self.lidar_points_x, self.lidar_points_y)))

        # Log the save operation
        self.get_logger().info(f"Map saved as {save_path_png}")
        self.get_logger().info(f"LiDAR points saved as {save_path_npy}")
